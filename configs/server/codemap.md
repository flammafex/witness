...
| `/v1/timestamp` | exact | `{{ADMIN_IP}}`, `{{CLIENT_IP}}` | **create** (write) |
| `/v1/timestamp/{hex-hash}` | regex `[a-fA-F0-9]+` | **public** | read-only retrieval |
...
- Note the asymmetry: gateway **read** paths are public (good — anyone should verify), while gateway **write** paths (`/v1/timestamp`, `/ws/events`) and all witness **sign** paths are gated.
