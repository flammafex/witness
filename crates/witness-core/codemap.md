...
- **BLS via `blst::min_sig`** (48-byte compressed G1 signatures, 96-byte compressed G2 public keys) with a fixed domain-separation tag (`WITNESS_BLS_SIG_…`); secret-key seed material zeroized after `key_gen`; keys generated from `OsRng`.
...
- **witness-cli** (HTTP client): offline `verify_proof_bundle` with `ProofVerificationConfig`, `NetworkVerificationConfig` parsing, `ProofBundle` display, anchor-provider display, `FreebirdToken`, attestation-job request/response types.
- **witness-auditor** (RFC 9162 walker): `verify_signed_tree_head`, `verify_log_consistency`, `SignedTreeHead`, `TreeHead`, `LogConsistencyProof`, `NetworkVerificationConfig`.
