...
Runnable example Witness network demonstrating **BLS12-381 signature aggregation** — the variant where 3 individual witness signatures collapse into 1 aggregated signature instead of 3 separate Ed25519 signatures. Follows the standard lifecycle (`setup.sh` → `start.sh` → `demo.sh` → `stop.sh`) with a threshold-2, 3-witness network whose explicit purpose is to show the ~50% attestation-size savings (1 × 96-byte BLS signature vs 3 × 64-byte Ed25519 signatures = 192 bytes). A local `README.md` documents the BLS variant in depth.
...
- **demo.sh**: pitches the comparison (Ed25519: 3 sigs = 192 bytes; BLS: 1 sig = 96 bytes, ~50% savings, single pairing verification), timestamps a test file through the `witness` CLI, `get`s the attestation JSON, and asserts the signature shape is `signatures.Aggregated` (extracts `.signers` and hex signature byte length, computes savings vs 192), then `verify`s the attestation.
...
3. `demo.sh` — create `test-file.txt`, timestamp via CLI with `--save`, `get` by hash and inspect the aggregated-signature JSON (assert `.signatures.Aggregated`), compute byte savings, `verify` the saved attestation.
