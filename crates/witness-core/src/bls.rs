...
pub fn verify_signature_bls(
    ...
    // Parse signature
    let signature =
        Signature::from_bytes(signature_bytes).map_err(|_| WitnessError::InvalidSignature)?;
...
pub fn aggregate_signatures_bls(signature_bytes_list: &[Vec<u8>]) -> Result<Vec<u8>> {
    ...
    for sig_bytes in &signature_bytes_list[1..] {
        let sig = Signature::from_bytes(sig_bytes).map_err(|_| WitnessError::InvalidSignature)?;
...
pub fn verify_aggregated_signature_bls(
    ...
    let signature =
        Signature::from_bytes(aggregated_signature).map_err(|_| WitnessError::InvalidSignature)?;
    ...
    for pk in &public_keys[1..] {
        agg_pubkey.add_public_key(pk, true).map_err(|_| {
            WitnessError::InvalidPublicKey("Failed to aggregate public keys".to_string())
        })?;
    }

    // Verify
    let result = signature.verify(true, &message, DST, &[], &agg_pubkey.to_public_key(), true);
...
mod tests {
    ...
    fn test_bls_signature_aggregation() {
        ...
        let public_keys: Vec<PublicKey> = keys.iter().map(|(_, pk)| *pk).collect();
    ...
    fn test_bls_aggregation_wrong_keys() {
        ...
        let wrong_keys: Vec<PublicKey> = (0..3).map(|_| generate_bls_keypair().1).collect();
